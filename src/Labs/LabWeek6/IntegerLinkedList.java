...
public class IntegerLinkedList {
	...
	private void doubleValuesRecursiveHelper(IntegerNode current) {
		if (current == null) {
			return;
		} else {
			...
			// add one to the elements in the REST of the list
			doubleValuesRecursiveHelper(current.next);
		}
	}
	...
	private void doubleOddValuesRecursiveHelper(IntegerNode current) {
		if (current == null) {
			return;
		}
		else {
			...
			doubleOddValuesRecursiveHelper(current.next);
		}
	}
	...
	private int product(IntegerNode current) {
		if(current == null) {
			return 1;
		}
		else {
			int first = current.getData();
			int productrest = product(current.next);
	...
	private boolean allNegativeRecursiveHelper(IntegerNode current) {
		if(current == null) {
			return true;
		}
		if(current.data >= 0) {
		...
		return allNegativeRecursiveHelper(current.next);
	}
	...
	public boolean isSortedAscending() {
		// TODO: implement and test this method
		return isSortedAscendingRecursiveHelper(head, -99999999);
	}  
	
	
	private boolean isSortedAscendingRecursiveHelper(IntegerNode current, int before) {
		if(current == null) {
			return true;
		}
		...
		return isSortedAscendingRecursiveHelper(current.next, current.data);
	}
